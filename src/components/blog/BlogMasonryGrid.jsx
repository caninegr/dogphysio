import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import Masonry from "react-masonry-css";
import { blogContent } from "../../data/blogs";

const breakpointColumnsObj = {
  default: 4,
  1100: 3,
  768: 2,
  500: 1,
};

const BlogMasonryGrid = ({ searchQuery }) => {
  const [filtered, setFiltered] = useState(blogContent);
  
  // Sort function: newest to oldest
  const sortByDate = (posts) => {
    return [...posts].sort((a, b) => {
      // If sortDate exists, use it for sorting
      if (a.sortDate && b.sortDate) {
        return new Date(b.sortDate) - new Date(a.sortDate);
      }
      // If sortDate doesn't exist, maintain original order
      return 0;
    });
  };

  useEffect(() => {
    let items;
    if (searchQuery) {
      items = blogContent.filter(
        (elm) =>
          elm.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
          elm.description.toLowerCase().includes(searchQuery.toLowerCase())
      );
    } else {
      items = blogContent;
    }
    // Apply date sorting
    setFiltered(sortByDate(items));
  }, [searchQuery]);

  return (
    <>
      <Masonry
        breakpointCols={breakpointColumnsObj}
        className="my-masonry-grid"
        columnClassName="my-masonry-grid_column"
      >
        {filtered.map((item, i) => (
          <div className="grid-item" key={i}>
            {/* <!--Blog Post--> */}
            <article className="ptf-post ptf-post--style-2">
              <div className="ptf-post__media">
                <img
                  src={`/assets/img/blog/masonry/${item.img}.webp`}
                  alt=""
                  loading="lazy"
                />
                <div className="ptf-post__media__content">
                  <header className="ptf-post__header">
                    <div className="ptf-post__meta">
                      <span className="cat">{item.cat}</span>
                      <span className="date">{item.date}</span>
                    </div>
                    <h3 className="ptf-post__title">{item.title}</h3>
                  </header>
                </div>
              </div>
              {/* End .ptf-post */}
              <div className="ptf-post__content">
                <header className="ptf-post__header">
                  <div className="ptf-post__meta">
                    <span className="cat">{item.cat}</span>
                    <span className="date">{item.date}</span>
                  </div>
                  <h3 className="ptf-post__title">
                    <Link to={`/blog-article/${item.id}`}>{item.title}</Link>
                  </h3>
                </header>
                <div className="ptf-post__excerpt">
                  <p>{item.description}</p>
                </div>
                <footer className="ptf-post__footer">
                  <Link
                    className="ptf-read-more-link"
                    to={`/blog-article/${item.id}`}
                  >
                    Διαβασε
                  </Link>
                </footer>
              </div>
              {/* End .ptf-post__content */}
            </article>
          </div>
        ))}
      </Masonry>
      {!filtered.length && (
        <div
          style={{
            backgroundColor: "#fff",
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
            fontSize: "20px",
          }}
        >
          Δεν βρέθηκαν αποτελέσματα.
        </div>
      )}
    </>
  );
};

export default BlogMasonryGrid;